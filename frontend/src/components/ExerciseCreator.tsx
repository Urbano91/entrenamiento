...
export const ExerciseCreator: React.FC<Props> = ({ exerciseId, onClose, onExerciseReady }) => {
    ...
    const checkSimilar = async () => {
        ...
        try {
            ...
            const message = reason instanceof Error ? reason.message : 'No se pudo Buscars parecidos';
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-x-hidden bg-slate-950/70 p-0 backdrop-blur-sm sm:p-4" onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
            <div role="dialog" aria-modal="true" aria-label="Crear" className="flex h-[100dvh] max-h-[100dvh] min-w-0 w-full max-w-4xl flex-col overflow-hidden bg-white shadow-2xl sm:h-auto sm:max-h-[94dvh] sm:rounded-2xl" onClick={event => event.stopPropagation()}>
                <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:px-6 sm:py-4">
                    <div>
                        ...
                        <h2 className="mt-1 text-xl font-bold text-slate-950">{exerciseId ? 'Editar' : 'Crear'}</h2>
