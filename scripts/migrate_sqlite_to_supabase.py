...
def translate_default(default: Optional[str]) -> Optional[str]:
    if default is None:
        return None
    value = default.strip()
    ...
    if re.fullmatch(r"'(?:''|[^'])*'", value, flags=re.DOTALL):
        return value
    raise ValueError(f"Default SQLite no soportado de forma segura: {default!r}")
    ...
    for table_name in topological_order(tables):
        ...
        for column in table.columns:
            ...
            if column.not_null or column.pk_order:
                definition += " NOT NULL"
            translated_default = translate_default(column.default)
        ...
        for index in table.indexes:
            if index.origin != "c":
                continue
            if index.partial:
                raise ValueError(f"Índice parcial no soportado: {index.name}")
            ...
            index_statements.append(
                ...
                f"ON {schema}.{postgres_identifier(table.name)} ({columns});"
            )
    ...
    for table in result.tables:
        ...
        for column in table.columns:
            ...
            try:
                ...
                translate_default(column.default)
        ...
        for index in table.indexes:
            if index.partial:
                result.errors.append(f"{table.name}.{index.name}: índice parcial no soportado")
